...
const GuildSettings = require("../database/schemas/Guild");
...
module.exports = async(client) => {
  ...
  app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
    ...
    let storedSettings = await GuildSettings.findOne({ id: guild.id });
    if (!storedSettings) {
      ...
      storedSettings = await GuildSettings.findOne({ id: guild.id });
    }

    let ownerUser = await guild.members.cache.get(guild.ownerId)
    let ownerName = ownerUser.user.tag

    let premiumStatus
    if(storedSettings.premium === true) {
      premiumStatus = "Activated <3"
    } else {
      premiumStatus = "No active subscriptions"
    }

    renderTemplate(res, req, "newDash.ejs", {
      guild,
